(function(ng, moment, _, undefined) {
    ...
    ng.module('app.controllers')
        .controller('MonitorCategoriesController', ["$scope", "$http", "GlobalsService", 
            function($scope, $http, GlobalsService) {
        var vm = this;

		$scope.title = 'Listado de las categorías de los monitores';
        ...
        function paginatorCallback(page, pageSize) {
            ...
            return $http.get(GlobalsService.getAPIURL() + '/api/monitorCategories?' + $.param( params ) ).then(function(result){
